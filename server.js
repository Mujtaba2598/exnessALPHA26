...
// ⭐ PRIMARY API KEY - Your provided key
...
async function fetchBalance(accountId) {
    try {
        ...
        // ✅ FIXED: Use try-catch with timeout
        let accountInfo;
        try {
...
async function shouldClosePositionAI(position, accountId) {
    try {
        ...
        try {
            ...
            return { shouldClose: false, reason: 'Timeout', profitPercent: 0, currentPrice: 0 };
        }
        ...
        try {
            ...
            // If can't get data, use simple profit/loss logic
            if (profitPercent >= 2) {
...
app.get('/api/debug-balance', authenticate, async (req, res) => {
    try {
        ...
        res.json({
            ...
            equity: result.full?.equity || 0,
            margin: result.full?.margin || 0,
            freeMargin: result.full?.freeMargin || 0,
            apiKeyStatus
        });
    } catch (error) {
    ...
    try {
        ...
        if (!ticker) {
            return res.status(500).json({ 
                success: false, 
                message: 'TickerAll not initialized. Please check API key status in admin panel.' 
            });
        }
        ...
        // Try to start session with timeout
        let accountId;
        try {
